...
async function addGroupMember (currentUser, groupId, data) {
  logger.debug(`START: addGroupMember - Group = ${groupId} Criteria = ${data}`)
...
async function addGroupMemberBulk (currentUser, groupId, data) {
  logger.debug(`START: addGroupMemberBulk - Group = ${groupId} Data = ${JSON.stringify(data)}`)
 ...
 */
async function deleteGroupMember (currentUser, groupId, memberId, criteria) {
  logger.debug(`START: deleteGroupMember - Group = ${groupId} Member = ${memberId} Criteria = ${JSON.stringify(criteria)}`)
  const session = helper.createDBSession()
  const tx = session.beginTransaction()
  const isAdmin = currentUser === 'M2M' || helper.hasAdminRole(currentUser)
  ...
  try {
    ...
    const universalUID = criteria ? criteria.universalUID : null
...
async function deleteGroupMemberBulk (currentUser, groupId, data) {
  logger.debug(`START: deleteGroupMemberBulk - Group = ${groupId} Data = ${JSON.stringify(data)}`)
...
async function getGroupMembers (currentUser, groupId, criteria) {
  logger.debug(`START: getGroupMembers - Group = ${groupId} Criteria = ${JSON.stringify(criteria)}`)
  const session = helper.createDBSession()
  const isAdmin = currentUser === 'M2M' || helper.hasAdminRole(currentUser)
...
async function getGroupMemberWithSession (session, groupId, memberId) {
  logger.debug(`START: getGroupMemberWithSession - Group = ${groupId} Member = ${memberId}`)
  try {
...
async function getGroupMember (currentUser, groupId, memberId) {
  logger.debug(`START: getGroupMember - Group = ${groupId} Member = ${memberId}`)
  const isAdmin = currentUser === 'M2M' || helper.hasAdminRole(currentUser)
  const session = helper.createDBSession()
  try {
    const group = await helper.ensureExists(
 ...
 * @param {String} groupId the group id
 * @param {Object} criteria the query parameters
 * @returns {Object} the group members count data
 */
async function getGroupMembersCount (groupId, criteria) {
  logger.debug(`START: getGroupMembersCount - Group = ${groupId} Criteria = ${JSON.stringify(criteria)}`)
  const session = helper.createDBSession()
  try {
    const group = await helper.ensureExists(session, 'Group', groupId)
    groupId = group.id
 ...
 */
async function listGroupsMemberCount (criteria) {
  logger.debug(`START: listGroupsMemberCount - Criteria = ${criteria}`)
  const session = helper.createDBSession()
  try {
    let queryToExecute = ''
 ...
 * @returns {Object} the search result
 */
async function getMemberGroups (memberId, criteria) {
  logger.debug(`START: getMemberGroups - Member = ${memberId}`)
  const session = helper.createDBSession()
  try {
    const returnUuid = criteria.uuid
...
async function groupValidityCheck (memberId, groupId) {
  logger.debug(`START: groupValidityCheck - Group = ${groupId} Member = ${memberId}`)
  const session = helper.createDBSession()
  try {
    const res = await session.run(
